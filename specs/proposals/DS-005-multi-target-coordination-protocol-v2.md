...
4. 首个合法声明到达后进入 250 ms 仲裁窗口；窗口内只有一个 endpoint 声明时绑定该目标。
5. 多个不同 endpoint 声明时判定冲突，发送相应 `cancelled`，不执行 DDC并提示用户手动选择。
6. 建议目标发现总窗口为 3 秒；没有声明时不猜测、不切屏，提示用户手动选择。
7. 绑定唯一目标后发送 `handover_request(intent = input_handover)`，继续目标唤醒、确认、DDC 和 `committed` 流程。
...
| `pairingCode` 或认证字段 | 见安全决定 | 是 | 详细认证模式待批准 |
...
- 最近合法消息在线窗口保持 6 秒。
- 多目标发现总窗口建议 3 秒；首个声明后的仲裁窗口建议 250 ms。
- 同一 `type + eventID + sourceEndpointID` 在有效期内视为重复。
- 每个事件最多绑定一个目标、执行一次 DDC 和发送一次逻辑提交。
- 已绑定目标后，其他 endpoint 的迟到 `input_present` 不得覆盖目标。
- 冲突、取消、配置重载、协同关闭或输入返回源端时清理所有相关计时器和待处理事件。
- 未找到唯一目标时禁止退化为列表第一项或最近使用项。
...
1. **建议：HMAC-SHA256。** 配对码不直接上网，使用标准 KDF 派生认证密钥，对规范化消息、时间戳和 endpoint 身份签名，并继续使用时间窗和重放缓存。
2. **兼容优先：沿用明文 pairingCode。** 实现较小，但安全模型仍仅适用于可信局域网，未来还需要再次升级协议。

本草案建议选择 HMAC，但规范化 JSON、KDF 参数、nonce、签名字段和公共密码学向量必须在用户批准安全方向后单独写全。未经批准不得自创算法或把配对码写入测试、日志和提案样例。
...
| P-004 | 多配置仅一个 input_present | 仲裁后绑定唯一 endpoint |
| P-005 | 仲裁窗口内两个 endpoint 声明 | 冲突取消，零 DDC |
...
1. v2 是否按建议直接采用 HMAC-SHA256，还是暂时沿用 v1 明文配对码。
2. 多目标发现总窗口是否采用 3 秒。
3. 首个声明后的仲裁窗口是否采用 250 ms。
4. 多目标发现超时或冲突时是否坚持“零 DDC、要求手动选择”（建议）。
