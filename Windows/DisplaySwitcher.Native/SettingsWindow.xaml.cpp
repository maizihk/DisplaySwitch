...
namespace winrt::DisplaySwitcher::Native::implementation
{
    ...
    UIElement SettingsWindow::BuildContent()
    {
        ...
        checkNetwork.Click([this](auto const&, auto const&)
        {
            ...
            checkNetworkAccess_(config, [this, lifetime](bool ready, std::wstring const& message)
            {
                validation_.Text(message); validation_.Visibility(Visibility::Visible);
                if (!ready) SetConnectionStatus(L"网络权限未就绪", false);
            });
        });
        auto detectSelected = Button(); detectSelected.Content(box_value(L"检测连接"));
        detectSelected.Click([this](auto const&, auto const&)
        {
        ...
        peerActions.Children().Append(checkNetwork); peerActions.Children().Append(detectSelected);
