...
- 实现：新增可复现的软件渲染模块，只为 Shell 生成透明的黑/白“显示器 + 百分号”线稿；按用户实机反馈将主体从 82% 调整为占 88% 图标槽。`AppIcon.ico` 及 EXE、设置窗口、任务栏应用图标不变。
...
- 自动验证：纯测试覆盖 light→black、dark→white、未知 fallback、16/20/24/32 像素透明边界与约 12% 光学留白、重复相同状态不刷新及主题切换一次更新。上一版 PR #74 CI run `33715488641` 已完成 x64 Release、368 checks、1.84 MiB 分发校验和 9 文件 artifact 上传；88% 主体版本以本次最终 CI 结果为准。
