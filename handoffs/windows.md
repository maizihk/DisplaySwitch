...
- 检测响应窗口修复提交：`c667cdde1e44c1ec1e2e780c0b7fe248807705d9`
...
- 实机出现“无响应”的直接原因是检测会话原先在点击时就启动 2 秒超时，而 PBKDF2、地址解析和实际 UDP 发送已移到后台；这些发送前工作会消耗甚至用尽响应窗口。现在只有 `SendRaw` 确认数据报发送成功后才开始完整 2 秒响应窗口，解析或发送失败则立即返回“探测发送失败”，不再误报为对端无响应。
- 新增脱敏阶段诊断，仅记录检测开始、发送成功与耗时、收到响应、认证结果或响应超时；不记录地址、端口、endpoint、eventID 或配对密码。
...
- 检测响应窗口修复后再次完整运行 `Windows/build-windows.ps1`：主程序、启动器、测试、全部公共向量和产物检查通过，`Windows/dist/` 绿色版目录为 1.66 MiB。新增慢发送前工作测试证明发送前耗时不占用响应窗口，取消后的发送仍为零数据报。
