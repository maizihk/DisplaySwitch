...
- 实现提交：`26c1e93`；本轮评审修复提交：`a6aeb21054a0bd599c79d42c80529028c61af757`
- 当前已验证实现 HEAD：`a6aeb21054a0bd599c79d42c80529028c61af757`
...
- USB 学习结果和待选设备按稳定 profile ID 隔离；切换编辑配置时读取各自 `triggerDevices`，学习弹窗取消、设置取消、保存和删除配置均不会覆盖其他配置的绑定。
- DS-005 v2 运行时尚未实施时，旧 v1 运行时只接受唯一已开启配置；多个已开启配置会停止 UDP 与自动 USB 监控，状态机关闭，并在发送、唤醒和 DDC 回调处再次阻断副作用。手动菜单仍只使用对应 profile 的本机显示器映射。
...
- Xcode 27 Beta 6（`/Users/maizi/Downloads/Xcode-beta.app`）：
  ...
  - XCTest bundle：36 项，0 失败。
  - 本机配置测试：23 项，覆盖 C-001 至 C-015 的 macOS 适用路径、0/1/多显示器、迁移、孤立映射、重排、非法范围、持久安全状态、peer identity 确认，以及本轮菜单完整性、USB profile 隔离、多 enabled 配置 v1 零副作用和单迁移配置 v1 兼容回归。
...
- `git diff --check`：通过。
- Xcode 27 Beta 6 的 `xcodebuild test` 在当前中文路径下错误报告找不到实际存在的 test bundle executable；使用同一 Xcode 构建出的 `xctest` 直接执行 bundle，完整 36 项测试通过。这是测试启动器问题，不是编译或测试失败。
- GitHub Actions macOS run [`33084700266`](https://github.com/maizihk/DisplaySwitch/actions/runs/33084700266) 在当前已验证实现 HEAD `a6aeb21054a0bd599c79d42c80529028c61af757` 上完整通过：Debug、36 项 XCTest、Release 打包、artifact 检查和严格 codesign 均成功。

## 尚需实机验证

- 设置页添加、删除、排序、重命名多个配置及菜单即时刷新。
- 真实 USB 学习后触发引用写入所选配置，以及在两个配置间切换、取消和保存时不串值。
