...
- 最终实现提交：`2f4185ddbcdb0c8e6c427d368cc5bcf43e0af204`
...
- 隔离 `UserDefaults` 和可替换失败存储共 14 项配置测试通过：
  - v1 数组成功迁移且原数据未覆盖。
  - 旧双显示器键成功迁移。
  - schema v2 正常读写。
  ...
  - 全新安装和替换显示器仍不猜测输入源。
- PR #15 GitHub Actions run `33061856839` 在实现提交 `2f4185ddbcdb0c8e6c427d368cc5bcf43e0af204` 上完整通过：
  - Debug：`BUILD SUCCEEDED`。
  - 全部 XCTest：27 项，0 失败；包含 14 项配置测试以及现有 17 条消息向量、16 条状态机向量。
