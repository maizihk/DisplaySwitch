...
- [x] 设置展示/AppKit 契约测试 37/37 通过；完整 XCTest 183/183 通过；`build-app.sh` 与严格 codesign 验证通过。
...
| 2026-09-01 | DS-025 USB 与协同设置布局对齐 | 自动验证完成，GUI 待验 | 6181b2a、10a3aa9、c08bcbb、3d42c4d、df7b02b、c0ce372、b98aa42、ecbce6c、本任务提交 | USB/协同各收敛为两组可滚动内容；修复尾部对齐、卡片深浅色层级/圆角、普通按钮样式、双背景画布、显示器控制卡片顶部孤立横线、配置详情左基线、无用上移/下移按钮、USB/协同共用的对端输入源两列布局、配置名称弹性宽度，以及仅由真实协同保存事件触发的窗口固定短暂状态反馈；设置展示/AppKit 37/37、完整 XCTest 183/183、Release 构建、Apple Development 严格验签与 ZIP 完整性通过；真实 GUI/辅助功能待验 |
