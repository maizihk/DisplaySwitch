...
- [x] UDP 主动发送统一绑定本机 `listenPort` 作为源端口，配置的 `peerPort` 仅作为目标端口；覆盖探测、协同唤醒、手动交接及其他主动发送，重配/停止会释放监听和发送连接，回复继续复用收到数据报的连接。
...
- [x] 原生读取失败不会显示为本次读取成功；如有历史可信缓存，只以“上次可信值”状态展示且不覆盖缓存。
...
- [x] 原生读取诊断显示脱敏的最后拒绝原因、实际 offset 与有限尝试次数；HDMI 固定 offset 0、Type-C/DP 固定 offset 0x51，不进行双 offset 盲探。
...
| 2026-08-29 | DS-009 Apple Silicon 原生显示控制 | 自动验证完成，实机待验 | 本任务提交 | 70 项 XCTest、Debug/Release、打包和严格验签；读取门控、映射幂等、Type-C 双写、M4 endpoint 分类和详细读取诊断回归通过，真实原生 DDC 未执行 |
