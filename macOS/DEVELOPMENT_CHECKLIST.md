...
### DS-017 内建 HDMI 生产读取事务（自动验证完成，实机待验）
...
- [x] 54 项 DDC 专项和 139 项完整 XCTest 通过；Release `build-app.sh`、打包与严格 codesign 验证通过。
- [ ] 小米内建 HDMI 连续读取及 Dell C2DP 不回归仍需用户使用 DS-017 测试包验证；验证前不得合并。
...
| 2026-08-30 | DS-017 内建 HDMI 生产读取事务 | 自动验证完成，实机待验 | 本任务提交 | HDMI 使用 8 次有界完整单写单读事务；Type-C/DP 与全部写入路径不变；54 项 DDC、139 项完整 XCTest、Release 打包和严格验签通过 |
