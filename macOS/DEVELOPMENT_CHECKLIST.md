...
### DS-016 内建 HDMI DDC 读取时序对齐（自动验证完成，实机待验）

- [x] 静态核对已验证第三方实现后确认：内建 HDMI 同样使用 `IOAVService`、chip `0x37`、写地址 `0x51`、读 offset `0`、11 字节回复和严格 checksum；此前尚未对齐的变量是读取时序与有限重试。
- [x] 内建 HDMI Get VCP 改为每次双写后等待 10 ms，最多执行 4 次严格事务，失败事务之间等待 5 ms；首次严格成功立即停止。
- [x] 每次事务使用全新清零的 11 字节回复缓冲区；保持严格长度、checksum、source、opcode、result 和 command 校验，不接受坏校验和或移位回复。
- [x] 不改 chip、offset、Set VCP、输入源切换、Type-C/DP、service 拓扑绑定、USB、网络或协议。
- [x] 55 项 DDC 专项和 140 项完整 XCTest 通过；Release `build-app.sh`、打包、解压验签及严格 codesign 通过。
- [ ] M4 mini 内建 HDMI 需实测显式读取；只有严格 DDC/CI 回复成功才算根因确认，失败仍显示上次可信值且不制造读数。
...
| 2026-08-30 | DS-016 内建 HDMI DDC 读取时序对齐 | 自动验证完成，实机待验 | 本任务提交 | 保持标准帧和严格校验，仅将 HDMI 读取对齐为 10 ms 等待、4 次上限、5 ms 间隔；55 项 DDC、140 项完整 XCTest、Release 打包和严格验签通过 |
