...
### DS-016 DDC 读取请求单写事务（自动验证完成，实机待验）
...
- [x] 读取事务改为每次尝试严格执行一次请求写入、一次回复读取；有限重试重复完整事务。纯 Set VCP 和输入源写入继续使用既有双写与任一成功聚合，不受读取修复影响。
- [x] 保持现有 chip、offset、等待、重试上限、严格长度/checksum/source/opcode/result/command 校验、service 拓扑绑定、USB、网络和协议不变。
- [x] 纯测试锁定“期待回复时 1 个写周期、纯写时 2 个写周期”；55 项 DDC 专项和 140 项完整 XCTest 通过，Release `build-app.sh`、打包、解压验签及严格 codesign 通过。
- [ ] M4 mini 内建 HDMI 需连续显式读取确认；只有严格 DDC/CI 回复稳定成功才算根因确认。两台 Type-C/DP 显示器读取及输入源写入需做最小实机回归。
...
| 2026-08-30 | DS-016 DDC 读取请求单写事务 | 自动验证完成，实机待验 | 本任务提交 | 时序单独实测无改善后定位到重复 Get VCP 请求；读取改为单写单读、纯写保留双写，55 项 DDC、140 项完整 XCTest、Release 打包和严格验签通过 |
