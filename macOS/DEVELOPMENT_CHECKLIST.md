...
### DS-016 DDC 读取请求按传输路径分流（自动验证完成，实机待验）

- [x] 第一阶段仅将内建 HDMI 读取对齐为 10 ms 等待、4 次上限和 5 ms 间隔；用户实测没有改善，已经排除“只改时序即可解决”的判断。
- [x] 当前第三方运行时实测与本项目使用相同的 service、chip `0x37`、写地址 `0x51`、读 offset `0`、11 字节回复和严格 checksum；但每次 Get VCP 只执行一次 WriteI2C 和一次 ReadI2C，本项目此前错误地执行两次相同请求写入后只读一次回复。
- [x] 首个测试包把所有读取统一改为单写；用户实测原本可读的 Type-C/DP 立即稳定退化为 `read-reply-rejected/null-reply`，因此已否定“所有传输路径都单写”的实现并将该包作废。
- [x] 读取请求按当前 transport 明确分流：内建 HDMI 每次尝试一写一读，Type-C/DP 恢复既有双写一读；纯 Set VCP 和输入源写入继续使用双写与任一成功聚合。
- [x] 保持现有 chip、offset、等待、重试上限、严格长度/checksum/source/opcode/result/command 校验、service 拓扑绑定、USB、网络和协议不变。
- [x] 纯测试锁定“HDMI 读取 1 个写周期、Type-C/DP 读取 2 个写周期、纯写 2 个写周期”；55 项 DDC 专项和 140 项完整 XCTest 通过，Release `build-app.sh`、打包及严格 codesign 通过。
- [ ] 先确认原本可读的 Type-C/DP 显示器恢复严格读取，再连续验证 M4 mini 内建 HDMI；只有严格 DDC/CI 回复稳定成功才算 HDMI 根因确认。输入源写入仍需最小实机回归。
...
| 2026-08-30 | DS-016 DDC 读取请求按传输路径分流 | 自动验证完成，实机待验 | 本任务提交 | 全局单写造成 Type-C/DP `null-reply` 回归后已作废；现为 HDMI 单写、Type-C/DP 双写、纯写双写，55 项 DDC、140 项完整 XCTest、Release 打包和严格验签通过 |
