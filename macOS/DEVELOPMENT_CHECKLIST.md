...
- [x] 同一切换事件的不同显示器并发写入；同一 selector 去重并保持串行，且只与该显示器的普通原生 DDC 共用输入优先的窄 I2C 仲裁，不同显示器互不阻塞。
...
| 2026-08-29 | DS-009 多显示器输入源并发 | 自动验证完成，实机待验 | 本任务提交 | 不同显示器并发、同一 selector 串行去重且与普通 DDC 互斥；15 项专项、107 项全部 XCTest、Debug/Release、打包和严格验签通过 |
