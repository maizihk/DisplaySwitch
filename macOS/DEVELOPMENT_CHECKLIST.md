...
- [x] 第十三轮安全复查发现映射完整性要求每台显示器都有输入源，并且整数校验接受 `0`；空字段因此被迫转换为危险哨兵。现 USB 与协同映射允许留空并按显示器跳过，至少一台存在 `1...65535` 映射才允许启用；USB 空映射继续按公共合同只报告 `missing_mapping` 且零硬件/网络副作用，0 单独报告 `invalid_mapping`。旧配置中的 `0` 在加载时归一为缺失，最终输入源服务、DDC 路由和原生写入口均有独立非零范围门控。
