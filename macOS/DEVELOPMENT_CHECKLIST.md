...
- [x] USB 学习与待选设备按协同配置隔离，切换编辑、取消或保存不会覆盖其他配置的触发绑定。
- [x] DS-005 v2 运行时完成前，多个已开启配置会暂停旧 v1 自动网络/USB 协同，并阻断其网络、唤醒和 DDC 副作用；单个迁移配置继续兼容 v1。
...
| 2026-08-27 | M-004B DS-004 本机多协同配置模型 | 自动验证完成，实机待验 | 26c1e93、a6aeb21 | 23 项配置测试、36 项全部 XCTest、Debug/Release、打包与严格验签通过；PR #20 当前实现 CI 通过 |
