...
DisplaySwitch 是一个面向多台 macOS / Windows 电脑共享显示器和 USB 键鼠场景的原生工具。macOS 菜单栏 App 与 Windows 托盘 App 可以分别控制显示器，也可以通过可信局域网协同：USB 切换完成后，目标端确认就绪，源端再执行 DDC/CI 输入源切换。
...
| Windows | C++ / WinUI 3 | 托盘控制、动态多显示器、亮度/对比度/音量、原生 DDC/CI 与 ControlMyMonitor、USB 自动切换、本机协同配置 | `Windows/build-windows.ps1` |
...
4. 为每台显示器选择 Windows 原生 DDC/CI 或 ControlMyMonitor，按需开启亮度、对比度、音量和显式回读；硬件链路不兼容时可切换后端。
5. 配置完成后再启用自动切换和登录启动。
